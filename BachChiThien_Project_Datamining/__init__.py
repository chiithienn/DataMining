...
class Spmf:
    ...
    def to_pandas_dataframe_for_FOSHU(self, pickle=False):
        """
        Convert output to pandas DataFrame
        pickle: Save as serialized pickle
        """
        ...
        for pattern in self.patterns_:
            itemsets = pattern[0].split("#UTIL:")[0].strip()
            utility = pattern[0].split("#UTIL:")[1].split("#RUTIL:")[0].strip()
            relative_utility = pattern[0].split("#RUTIL:")[1].strip()

            patterns_dict_list.append({'Itemsets': itemsets, 'Utility ($)': utility, 'Relative Utility': relative_utility})
